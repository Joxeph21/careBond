...
// // Configure NProgress to hide the spinner since NextTopLoader usually handles the UI
...
//       NProgress.done();
//     };
//   }, [query, debouncedUpdate, searchParams, searchKey]);
...
export default function SearchBox({
  ...
  // URL value (source of truth)
  const paramQuery = searchParams?.get(searchKey) ?? "";


  // Draft input value
  const [query, setQuery] = useState(paramQuery);

  // Sync URL → input without an effect (avoids cascading renders)
  const prevParamQuery = useRef(paramQuery);
  if (prevParamQuery.current !== paramQuery) {
    prevParamQuery.current = paramQuery;
    setQuery(paramQuery);
  }
  ...
  // Draft → URL (start progress immediately for typing feedback)
  useEffect(() => {
    if (query !== paramQuery) {
      debouncedUpdate(query);
    }
  ...
  }, [query, paramQuery, debouncedUpdate]);
  ...
  return (
    <div ref={dropdownRef} className="relative w-full max-w-87.5">
      <form
        ...
        className={`w-full gap-3 ring px-4.5 py-3 ring-[#E4E4E4] rounded-[10px] flex items-center transition-all duration-200 ${
          isFocused ? "ring-primary/40 shadow-sm" : ""
        } ${className}`}
      >
        <button type="submit" className="shrink-0 cursor-pointer">
          <Icon
      ...
      <AnimatePresence>
        {isGlobalSearchBar && isFocused && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 4, scale: 0.98 }}
            transition={{ duration: 0.15, ease: "easeOut" }}
            className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-card-shadow border border-gray-100 z-50"
          >
